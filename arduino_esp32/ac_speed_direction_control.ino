...
unsigned int reduccionDelay = 50;
...
void loop() {
  ...
  if (activo) {
    ...
    if (objetivoAlcanzado) {
      Serial.println("Objetivo alcanzado. Iniciando reducción progresiva de potencia...");
  
      // Proceso de reducción progresiva
      while (dimmerValue < maxDimmer) {
        dimmerValue += 100;  // Paso de reducción (ajustable)
        if (dimmerValue > maxDimmer) dimmerValue = maxDimmer;
  
        delay(reduccionDelay);
  
        // También puedes seguir leyendo el ángulo si lo deseas aquí (opcional)
        getAngle();
      }
  
      Serial.println("Potencia reducida completamente.");
      activo = false;
